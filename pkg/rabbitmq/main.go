...
func (rmq *RabbitMQ) Publish(queueName string, exchangeName string, data []byte) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err := rmq.ch.PublishWithContext(ctx, exchangeName, queueName, false, false, amqp.Publishing{
